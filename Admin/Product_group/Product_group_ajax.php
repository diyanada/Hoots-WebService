...
if($_GET['log']=="todbjsadd")
{
	...
	if( $conn === false ) {
	  ...
	  $status = 0;
	  $produt_id = 0;
	  $sql = "{CALL TYK_PRODUCT_GROUP_CREATE(?,?,?,?,?,?)}";
	  $params = array( 
                             array($_GET["pdgp_name"], SQLSRV_PARAM_IN),
                             array($_GET["pdgp_pass"], SQLSRV_PARAM_IN),
                             array($_GET["pdgp_au"], SQLSRV_PARAM_IN),
	  ...
	  if( $stmt === false ) {
	   echo "<strong style='color:#900'>Couldn't insert to the database<br />"; 
		...
		echo "<br /><input type='button' id='button' value='Update Query' onclick='run_querey()' /></strong>";
		  ...
		  if($status <0){
			  echo "<strong style='color:#900'>Couldn't insert to the database<br />"; 
			  echo "<strong style='color:#900'>Product group name already use<br />"; 
			  echo "<br /><input type='button' id='button' value='Update Query' onclick='run_querey()' /></strong>";
		  }
		  else{
...
if($_GET['log']=="todbjssearch")
{
	...
	$sql = "SELECT 	
		[NAME]
      ,[AU_CODE]
		,'<a target=\"fr_one\" href=\"Product_group_update.php?product_id='+CONVERT(varchar(10), [ID])+'\">Edit</a>' as 'Edit Product Type'
  						FROM [PRODUCT_GROUP]
	  						WHERE 1=1";
	 
	 if ($_GET['pdgp_name'] != "") $sql .= " AND [NAME] LIKE '%".$_GET['pdgp_name']."%'";
	 if ($_GET['pdgp_au'] != "") $sql .= " AND [AU_CODE] LIKE '%".$_GET['pdouty_words']."%'";
...
if($_GET['log']=="todbjsupdte")
{
	...
	if( $conn === false ) {
	  ...
	  $status = 0;
	  $produt_id = 0;
	  $sql = "{CALL TYK_PRODUCT_GROUP_UPDATE(?,?,?,?,?,?)}";
	  $params = array( 
                             array($_GET["pdgp_id"], SQLSRV_PARAM_IN),
                             array($_GET["pdgp_name"], SQLSRV_PARAM_IN),
							 array($_GET["pdgp_pass"], SQLSRV_PARAM_IN ),
							 array($_GET["pdgp_au"], SQLSRV_PARAM_IN ),
	   ...
	   echo "<strong style='color:#900'>Couldn't insert to the database<br />"; 
		...
		echo "<br /><input type='button' id='button' value='Update Query' onclick='run_querey()' /></strong>";
		  ...
		  if($status <0){
			...
			echo"<strong style='color:#009900'>Successfully Added</strong><br />";  
			echo "<a  href=\"Product_group_insert.php\">";
