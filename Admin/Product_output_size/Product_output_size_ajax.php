...
if($_GET['log']=="todbjsadd")
{
	...
	if( $conn === false ) {
	  ...
	  $status = 0;
	  $produt_id = 0;
	  $sql = "{CALL TYK_PRODUCT_OUTPUT_SIZE_CREATE(?,?,?, ?,?)}";
	  $params = array( 
                             array($_GET["pos_name"], SQLSRV_PARAM_IN),
                             array($_GET["pos_words"], SQLSRV_PARAM_IN),
...
if($_GET['log']=="todbjssearch")
{
	...
	$sql = "SELECT [NAME]
      ,[WORD]
	  ,'<a target=\"fr_one\" href=\"Product_output_size_update.php?product_id='+CONVERT(varchar(10), [ID])+'\">Edit</a>' as 'Edit Product Type'
  			FROM [PRODUCT_OUTPUT_SIZE]
	  		WHERE 1=1";
	 
	 if ($_GET['pos_name'] != "") $sql .= " AND [NAME] LIKE '%".$_GET['pos_name']."%'";
	 if ($_GET['pos_words'] != "") $sql .= " AND [WORD] LIKE '%".$_GET['pos_words']."%'";
...
if($_GET['log']=="todbjsupdte")
{
	...
	if( $conn === false ) {
	  ...
	  $status = 0;
	  $produt_id = 0;
	  $sql = "{CALL TYK_PRODUCT_OUTPUT_SIZE_UPDATE(?,?,?, ?,?)}";
	  $params = array( 
					  		 array($_GET["pos_id"], SQLSRV_PARAM_IN),
					  		 array($_GET["pos_name"], SQLSRV_PARAM_IN),
                             array($_GET["pos_words"], SQLSRV_PARAM_IN),
	   ...
	   echo "<strong style='color:#900'>Couldn't insert to the database<br />"; 
		...
		echo "<br /><input type='button' id='button' value='Update Query' onclick='run_querey()' /></strong>";
		  ...
		  if($status <0){
			...
			echo"<strong style='color:#009900'>Successfully Added</strong><br />";  
			echo "<a  href=\"Product_output_size_insert.php\">";
