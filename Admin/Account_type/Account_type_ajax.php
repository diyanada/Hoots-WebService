...
if($_GET['log']=="todbjsadd")
{
	...
	if( $conn === false ) {
	  ...
	  $status = 0;
	  $produt_id = 0;
	  $sql = "{CALL TYK_ACCOUNT_TYPE_CREATE(?,?,?, ?,?)}";
	  $params = array( 
                             array($_GET["act_name"], SQLSRV_PARAM_IN),
                             array($_GET["nofh"], SQLSRV_PARAM_IN),
	  ...
	  if( $stmt === false ) {
	   echo "<strong style='color:#900'>Couldn't insert to the database<br />"; 
		...
		echo "<br /><input type='button' id='button' value='Update Query' onclick='run_querey()' /></strong>";
		  ...
		  if($status <0){
			  echo "<strong style='color:#900'>Couldn't insert to the database<br />"; 
			  echo "<strong style='color:#900'>Account type name already use<br />"; 
			  echo "<br /><input type='button' id='button' value='Update Query' onclick='run_querey()' /></strong>";
		  }
		  else{
...
if($_GET['log']=="todbjssearch")
{
	...
	$sql = "SELECT [NAME]
      ,[NO_OF_FREE_HOOTS]
	  ,'<a target=\"fr_one\" href=\"Account_type_update.php?product_id='+CONVERT(varchar(10), [ID])+'\">Edit</a>' as 'Edit Product Type'
  			FROM [ACCOUNT_TYPE]
	  		WHERE 1=1";
	 
	 if ($_GET['act_name'] != "") $sql .= " AND [NAME] LIKE '%".$_GET['ctit_name']."%'";
	 if ($_GET['nofh'] != "") $sql .= " AND [NO_OF_FREE_HOOTS] = '".$_GET['nofh']."'";
...
if($_GET['log']=="todbjsupdte")
{
	...
	if( $conn === false ) {
	  ...
	  $status = 0;
	  $produt_id = 0;
	  $sql = "{CALL TYK_ACCOUNT_TYPE_UPDATE(?,?,?, ?,?)}";
	  $params = array( 
					  		 array($_GET["act_id"], SQLSRV_PARAM_IN),
					  		 array($_GET["act_name"], SQLSRV_PARAM_IN),
                             array($_GET["nofh"], SQLSRV_PARAM_IN),
	  ...
	  if( $stmt === false ) {
	   echo "<strong style='color:#900'>Couldn't insert to the database<br />"; 
		...
		echo "<br /><input type='button' id='button' value='Update Query' onclick='run_querey()' /></strong>";
		  ...
		  if($status <0){
			  echo "<strong style='color:#900'>Couldn't insert to the database<br />"; 
			  echo "<strong style='color:#900'>Account type name not valid<br />"; 
			  echo "<br /><input type='button' id='button' value='Update Query' onclick='run_querey()' /></strong>";
		  }
		  else{
			  
			echo"<strong style='color:#009900'>Successfully Added</strong><br />";  
			echo "<a  href=\"Account_typ_insert.php\">";
